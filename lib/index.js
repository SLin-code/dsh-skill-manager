	...
	return [
		exact(API.list, async (request, response) => {
			...
			try {
				...
				json(response, 200, {
					skills: (await mapConcurrent(snapshot.skills, LIST_CONCURRENCY, async (snapshotSkill) => {
						const skill = await view.registry.get(snapshotSkill.name, {
							cwd: view.cwd,
							scope: view.scope
						});
						return skill === void 0 ? void 0 : await entryOf(skill);
